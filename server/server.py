...
def auth(conn : socket.SocketType, msg : bytes):
    ...
    if len(tokens) != 3:
        conn.send(str.encode("<BAD>.Malformed auth string. <AUTH>.username.password format expected."))
        return

    authentic, res_msg = check_authfile(uname= tokens[1], pword= tokens[2])
    if authentic:
        update_auth_status(conn, True)
        conn.send(str.encode(f"<OK>.{res_msg}"))
    else:
        conn.send(str.encode(f"<BAD>.{res_msg}"))
    ...
    while cont_to_listen:
        try:
            ...
            if is_auth_req(msg):
                ...
                conn.send(str.encode("<BAD>.Not authenticated."))
...
rp_prefixes = ["<OK>", "<BAD>"] # Response prefixes for client to parse server messages
